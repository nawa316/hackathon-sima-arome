...
export default function QualityControlLayout({
  ...
  const menuItems = useMemo<DashboardMenuItem[]>(() => {
    const qcMenuItems = [
      ...
      {
        id: 'raw-qc',
        label: 'Raw Material Quality Control',
  ...
  const displayUserRoleFallback = useMemo(() => {
    ...
    return currentUser.admin_access ? 'Super Admin' : 'Quality Controller';
  }, [currentUser]);

  const [userName, setUserName] = useState<string>('QC Staff');
  const [userRole, setUserRole] = useState<string>('Quality Controller');
